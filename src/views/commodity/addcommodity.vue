 <template>
  ...
  <section style="padding:20px;position:relative">
    ...
    <el-form class ref="form" :model="form" label-width="120px">
      ...
      <el-form-item label="商品名称" crequired>
      ...
      <el-form-item label="商品图" style="width:900px" required>
        <draggable v-model="big_img">
          <transition-group>
            <img
              class="el-upload-list__item-thumbnail"
              v-for="img in big_img"
              :key="img"
              :src="img"
              alt
            />
      ...
      <el-form-item label="商品详情图" style="width:900px" required>
        <draggable v-model="img_json">
          <transition-group>
            <img
              class="el-upload-list__item-thumbnail"
              v-for="img in img_json"
              :key="img"
              :src="img"
              alt
            />
      ...
      <el-form-item style="width:900px" label="限购">
        ...
        <div v-show="limitBuyChecked">
          ...
          <div style="display:flex;align-items:baseline">
            ...
            <el-select style="margin:10px 10px 0 -14px;width:100px" v-model="periodType">
              <el-option
...
export default {
  data() {
    return {
      ...
      upload_qiniu_addr2: "http://video.pintaihui.cn/",
      qiniuData: { key: "", token: "" },
      qiniuData2: { key: "", token: "" },
      imageUrl: "",
      Global: {
  ...
  methods: {
    getQiniuToken: function() {
      const _this = this;
      axios.post(this.Global.dataUrl + "/qiniu/token").then(function(res) {
        if (res.data) {
      ...
      axios
        .post(this.Global.dataUrl + "/qiniu/video_token")
        .then(function(res) {
          if (res.data) {
            _this.qiniuData2.token = res.data.data.token;
          } else {
            _this.$message({
              message: res.data.msg,
              duration: 2000,
    ...
    handleAvatarSuccess2: function(res, file) {
      console.log(file)
      this.videosrc = this.upload_qiniu_addr2 + res.key;
      this.big_video = this.videosrc;
    },
    handleAvatarSuccess3: function(res, file) {
      this.img_json.push(file.url);
      const imgObj = new Image();
      imgObj.src = file.url;
      imgObj.onload = function() {
        console.log(this.width);
    ...
    getEditGoods() {
      ...
      getEditGoods(params)
        .then(res => {
          ...
          this.goods_feight_template_id = data.goods.goods_feight_template_id;
